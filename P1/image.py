...
class Image:
    ...
    def config(self, fileName):
        ...
        with open(fileName) as f:
            for line in f:
                ...
                imgData = ImageData()
                if (len(data) > 1):
                    if (len(data) == 3 and self.isValid(data)):
                        imgData.image = data[0]
                        imgData.noOfAvailable = data[1]
                        imgData.location = data[2]
    ...
    def show(self):
        tempDataList = []
        tempDataList.append(['Image', 'No of available', 'Location'])
        for name, imageData in self.imageDict.items():
            tempList = []
            tempList.append(imageData.image)
            tempList.append(imageData.noOfAvailable)
